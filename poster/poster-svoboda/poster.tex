...
The total length is close to 10 m, which was achieved by constructing a ``trombone-like'' turn as seen in the Figure. \\
...
\textbf{Analogue Analysis Circuit}
 ...
 A set of logical gates compares the trigger signal synchronised with the 500 kHz saw-tooth signal to the next root of the sine signal and deduces the current phase shift. This happens for every eighth trigger signal, so the time resolution of this method is only 62,5 kHz.
...
The amplified signal is digitized with a sampling frequency of 25 MHz and then processed on a computer. The data sample is then fitted to a function $f(t)=A \cdot \sin (2\pi\cdot 5e5 t + \varphi)$ and $\varphi$ is extracted and normalized. Only ranges of points close to $0$ are processed, because there the signal is assumed to be the least influenced by amplitude dampening.
 ...
 \headerbox{Signal Disturbances}{name=disturb,column=0,row=2,span=2,below=data}
 {
 %what why
 }<++>
...
\headerbox{References}{name=References,column=2,row=1, span=1,below=plans}
{
