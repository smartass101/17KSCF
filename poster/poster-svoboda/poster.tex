...
The total length of the new waveguide system is nearly 10 m, which was achieved by constructing a ``trombone-like'' turn as seen in fig %TODO
 ...
 The amplified sine-wave signal can be either analyzed in the analogue analysis circuit or digitized with the oscilloscope and then analyzed on the computer.
...
The amplified signal is digitized with a sampling frequency of 25 MHz and then processed on the computer. The data sample is then fitted to a function $f(t)=A \cdot \sin (2\pi\cdot 5e5 t + \varphi)$ and $\varphi$ is extracted and normalized. Only ranges of point close to $0$ are processed, because there the signal is assumed to be the least influenced by amplitude dampening.
 }
