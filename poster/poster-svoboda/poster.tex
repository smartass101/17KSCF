...
\begin{equation}
    ...
    \label{eq:ne}
\end{equation}
$\overline{n_e}$ is the electron density line-averaged over the distance $d$ that the MW with a wavelength in vacuum $\lambda_0$ travels through plasma


%needs smth about n_crit
$n_{crit}$ is the critical density above which the MW cannot propagate through plasma. $f \sim 75 \text{GHz} \dots n_{crit} \sim 6.9 \dot 10^{19} \text{cm}^{-3}$
\end{itemize}
\end{multicols}
 }
